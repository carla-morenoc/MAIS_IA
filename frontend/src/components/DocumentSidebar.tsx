...
export default function DocumentSidebar({ onOpenDocument, onDocumentsChange }: DocumentSidebarProps) {
  ...
  const handleManualTranscript = async () => {
    if (!manualVideoUrl || !manualVideoTitle || !manualTranscript) {
      setError("Indica la URL, el título y el archivo VTT del vídeo.");
      return;
    }
    ...
    try {
      ...
      setDocuments(data.map((d) => ({
        id: d.document_id, filename: d.filename, file_path: d.file_path,
        status: d.status, document_type: d.document_type || "pdf",
        totalChunks: d.total_chunks, errorMessage: d.error_message, is_active: d.is_active,
      })));
      startPolling(uploaded.document_id, uploaded.filename);
      setManualVideoUrl("");
      setManualVideoTitle("");
      setManualTranscript(null);
      showToast("Transcripción enviada para indexación.");
    } catch (err: any) {
      setError(err.message || "No se pudo importar la transcripción.");
    } finally {
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div className="pt-4 border-t border-zinc-900 space-y-4">
          <div className="flex flex-col gap-3">
            ...
            <div className="space-y-2 rounded-xl border border-zinc-900 bg-zinc-900/20 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Importar transcripción con tiempos</p>
              <input type="text" value={manualVideoUrl} onChange={(e) => setManualVideoUrl(e.target.value)} placeholder="URL del vídeo de YouTube" className="w-full bg-zinc-950 border border-zinc-800 text-zinc-200 text-xs rounded-lg px-3 py-2 outline-none focus:border-red-500/80" />
              <input type="text" value={manualVideoTitle} onChange={(e) => setManualVideoTitle(e.target.value)} placeholder="Título del vídeo" className="w-full bg-zinc-950 border border-zinc-800 text-zinc-200 text-xs rounded-lg px-3 py-2 outline-none focus:border-red-500/80" />
              <input type="file" accept=".vtt,.txt" onChange={(e) => setManualTranscript(e.target.files?.[0] || null)} className="w-full text-[10px] text-zinc-500" />
              <button type="button" onClick={handleManualTranscript} disabled={importingTranscript} className="w-full rounded-lg bg-red-950/40 border border-red-900/60 px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-red-300 disabled:opacity-40">
                {importingTranscript ? "Enviando..." : "Indexar transcripción"}
              </button>
            </div>
          </div>
