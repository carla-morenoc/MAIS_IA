...
/**
 * Activa o desactiva un documento para las búsquedas.
 */
export async function toggleDocument(documentId: string): Promise<ToggleDocumentResponse> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.detail || "No se pudo cambiar el estado del documento");
  }
...
/**
 * Importa una transcripción VTT creada fuera del VPS y la indexa como vídeo.
 */
export async function uploadYoutubeTranscript(
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.detail || "No se pudo importar la transcripción");
  }
