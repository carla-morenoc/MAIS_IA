...
def process_youtube_video_task(self, document_id: str):
    ...
    with sync_session_factory() as session:
        ...
        try:
            ...
            # Bloques cortos para que la cita lleve al tramo donde se explica el concepto.
            chunks_data = []
            chunk_index = 0
            texto_acumulado = []
            segundo_inicio = None
            ...
            # Reindexar debe sustituir los chunks anteriores, no acumularlos.
            client_sync.delete(
...
def _parse_vtt_transcript(transcript_text: str) -> list[tuple[float, str]]:
    """Extrae inicio y texto de cada bloque WebVTT."""
    ...
    for block in blocks:
        ...
        text = " ".join(line.strip() for line in lines if line.strip() and "-->" not in line)
...
@celery_app.task(name="MAIS_IA.process_youtube_transcript", bind=True, max_retries=3)
def process_youtube_transcript_task(self, document_id: str, transcript_text: str):  # noqa: ANN001
    """Indexa un VTT aportado manualmente, sin consultar YouTube desde el VPS."""
    with sync_session_factory() as session:
        document = session.get(Document, document_id)
        if document is None:
            return {"status": "error", "message": "Vídeo no encontrado"}
        try:
            ...
            if not entries:
                raise ValueError("No se encontraron bloques con timestamps en el VTT")
            ...
            for start, text in entries:
                if current_text and start - chunk_start >= 30:
                    chunks_data.append(ChunkData(
                        text=" ".join(current_text), doc_id=document_id,
                        filename=document.filename, page_number=int(chunk_start),
                        chunk_index=chunk_index,
                    ))
                    chunk_index += 1
                    current_text = []
            ...
            client.delete(
                collection_name=settings.qdrant_collection,
                points_selector=qdrant_models.Filter(must=[qdrant_models.FieldCondition(
            ...
            document.error_message = None
            session.commit()
            return {"status": "completed", "document_id": document_id, "total_chunks": len(chunks_data)}
        except Exception as exc:
            session.rollback()
            document = session.get(Document, document_id)
            if document is not None:
                document.status = DocumentStatus.FAILED
                document.error_message = f"Error al procesar la transcripción: {exc}"
                session.commit()
            logger.exception("Error indexando transcripción manual %s", document_id)
