...
@router.post(
    ...
    summary="Importar una transcripción de YouTube con tiempos",
)
...
async def upload_youtube_transcript(
    ...
    """Importa un VTT generado fuera del VPS y lo indexa como videotutorial."""
...
async def upload_document(
    ...
    session.add(document)
    # Confirmar antes de encolar: el worker debe poder leer el registro.
    await session.commit()
...
async def sync_youtube_videos(
    ...
    if payload and payload.channel_url:
        ...
        if video_match:
            ...
            if existe:
                ...
                if existe.status == DocumentStatus.PENDING:
                    await session.commit()
                    process_youtube_video_task.delay(str(existe.id))
                    return {"status": "success", "message": f"Vídeo '{existe.filename}' reencolado para indexación.", "added": 1}
    ...
    for entry in root.findall('atom:entry', ns):
        ...
        if not existe:
            ...
            session.add(nuevo_doc)
            queued_document_ids.append(str(new_id))
            added_count += 1
        elif existe.status == DocumentStatus.PENDING:
            queued_document_ids.append(str(existe.id))
            added_count += 1

    # Confirmar todos los cambios antes de publicar tareas en Redis.
    await session.commit()
